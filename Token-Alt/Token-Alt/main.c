...
/*  int main()
 *   function: program receives file through command line argument, opens and tokenizes file into array, separates tokens into several
 *   categories, counts total number of tokens in category, checks for duplicates, counts individual tokens and prints results
 *
 *   parameters: int argc - total number of arguments passed to the main function from the command line
 *               char *argv[] - char * array containing file name entered by user as command line argument to be opened by program
 */
int main(int argc, char *argv[])
{
    ...
    // Count lines in file to get total count of lines
    ...
    // Iterate through file and concatenate each line into string
    ...
    // Remove the file pointer from the string array
    for (i = 0; i < count; i++) {
        string[i] = string[i + 1];
    }
    
    // Add trailing line space to string
	strcat(string, "\n");
    
	//printf("The full string: %s\n", string);
    
    // Tokeninze string with a blank space as a delimiter
	char *tokenPtr = strtok(string, " ");
    
    // While tokenPtry is true, loop
	while (tokenPtr) {
		//printf("Token = %s\n", tokenPtr);
 ...
 *               char *tokArray[] - array of tokens in each token category
